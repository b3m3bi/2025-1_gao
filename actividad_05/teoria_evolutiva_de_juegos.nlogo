...
to init_pagos
  ;leer texto e interp como codigo
  set matriz_pagos read-from-string pagos ;guardamos la matriz de pagos
  set num_estrategias length matriz_pagos ; el num de estrategias es igual a la long de la matriz de pagos
  set colores map [k -> 15 + 30 * k] (range num_estrategias) ; mapeamos colores tantos como estrategias
end

to init_jugadores
  let distribucion_inicial read-from-string num_jugadores_por_estrategia
  ;verificamos si coinciden el num de estrategias con el tamanio de la lista del num de jugadores por estrategia
  if length distribucion_inicial != length matriz_pagos [
    ; con user-message enviamos un mensaje
    user-message "El número de elementos de num_jugadores_por estrategia debe ser igual al número de filas de la matriz de pagos "
  ]
  ;Para cada elemento de la lista del num de jug por est
  let i 0
  foreach distribucion_inicial [
    j ->
    ;creamos esa cantidad de tortugas
    create-turtles j [
      ;en una posicion aleatoria
      setxy random-xcor random-ycor
      ;cambiamos forma a ciculo
      set shape "circle"
      ;cambiamos tamaño
      set size 0.75
      ; tendra la estrategia i??????????????
      set estrategia i
      ;la estrategia sig por lo mientras es la anterior???????????
      set estrategia_siguiente estrategia
      ;pago por lo mientras es 0
      set pago 0
    ]
...
to init_grafica
  ;indicamos que grafica controlamos
  set-current-plot "Frecuencia estrategias"
  ;para cada estrategia
  foreach (range num_estrategias) [
    i ->
    ;creamos una pluma
    create-temporary-plot-pen (word i) ;con word pasamos a string un num
    ;cambiamos el color
...
to jugar
  ...
  ;accedemos al pago del elemento que correcponde de la lista de mi estrategia
...
;graf frecuencias de cada estrategia
...
;map aplica una funcion a una lista
