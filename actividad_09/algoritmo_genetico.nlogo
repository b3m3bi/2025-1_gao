;definimos subconjunto de agentes, plural singular
...
to setup
  clear-all
  ;inicializamos var
  set entradas-red ["PosX" "PosY" "DistBordeDer" "DistBordeIzq" "DistBordeAba" "DistBordeArr" "DensidadTortugasFrente" "Aleatorio" "Oscilador" "DensidadAlimentoFrente" "DensidadDepredadorFrente"]
...
to crear-genotipo
  ...
  repeat num-genes [
    let gen []
    ;lista donde agregamos el elemento aleatorio
    ;3 elementos
...
to go
   ...
   repeat duracion-generacion[
     ifelse trazo [ask tortugas [pendown ]][ask tortugas[penup]]
     ifelse Estrategia = "Algoritmo-Genetico"[
      ask tortugas [moverse]][ask tortugas[buscar-comida] ask tortugas[huir]]
     ask tortugas [comer]
...
to moverse
  ;Pedimos ejecuten las funciones de entrada
  let valores-entrada map runresult entradas-red ; recorro entradas-red y ejecute runresult que es tal cual llamar a la funcion
  foreach range length salidas-red [
    j ->
    let lista-entradas lista-pesos-entradas-salidas-i j
    let suma-pesos-entradas sum (map * lista-entradas valores-entrada);pesos
...
to-report lista-pesos-entradas-salidas-i [i]
  let lista-pesos n-values (length entradas-red) [ 0 ];lista de 0
  foreach genotipo [
    gen ->
    if (item 1 gen) = i ;si hay peso
...
;------------------------Funciones Entrada--------------------------------
to-report Posx
  ;retorna coordenada x de la tortuga, dividimos para que este entre 0 y 1
  report xcor / world-width
end

to-report Posy
  ;retorna coordenada y de la tortuga, dividimos para que este entre 0 y 1
  report ycor / world-width
end


to-report DistBordeDer
  ;distancia de la tortuga al borde de la derecha
  report (distancexy max-pxcor ycor) / world-width
end


to-report DistBordeIzq
  ;distancia de la tortuga al borde de la izq
  report (distancexy min-pxcor ycor) / world-width
end

to-report DistBordeArr
  ;distancia de la tortuga al borde de la arriba
  report (distancexy xcor max-pycor) / world-width
end

to-report DistBordeAba
  ;distancia de la tortuga al borde de la abajo
...
;;;;;;;;;;;ver red neuronal de tortuga
;para llamarla: ask one-of tortugas [ show obtener-fenotipo ]
...
;_________________________Funciones mi algoritmo_____________________________
to buscar-comida
  if not can-move? 1 [
    set heading 90
    fd 1
  ]
  let comida-cerca one-of alimentos in-cone radio 90
  if comida-cerca != nobody [
    face comida-cerca  ; girar en direccion del alimento
    fd 1
  ]
end

to huir
  ;evitamos que se atasque
  if not can-move? 1 [
    set heading 90
    fd 1
  ]
  ; Buscamos depredadores en el radio de visión de radio/2
  let depreda one-of depredadores in-cone (radio / 2) 90
  if depreda != nobody [
    face depreda
    bk 1
  ]
...
Estrategia
Estrategia
"Algoritmo-Genetico" "Algoritmo-Humano"
1

@#$#@#$#@
## WHAT IS IT?

Elegi la opcion 2 de crear mi propio algoritmo, para eso al final agregue dos funciones, buscar-comida que al igual que densidad comida busca comida en un cono de 90 grados, si hay entonces se gira hacia ella y avanza 1. Y la funcion huir detecta los depredadores en un area más pequenia, elige uno y retrocede. En ambas hay una linea que sirve para que no se atasque en los bordes.
