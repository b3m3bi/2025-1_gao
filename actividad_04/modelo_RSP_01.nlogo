...
to go
  ;elegimos una primera celda
  ask one-of patches[
    ;elegir al competidor
    let mi_competidor one-of other patches
    if tipo_interaccion = "local"[set mi_competidor one-of neighbors ]

    ;Revisamos cual gana
    ifelse le_gano? mi_competidor[
      ; si le gano, vemos si lo invado
...
to-report le_gano? [el_otro]
  (ifelse
   ...
   ; pcolor = red and ([pcolor] of el_otro = blue or [pcolor] of el_otro = yellow) and random-float 1.0 < p_r [report true]
